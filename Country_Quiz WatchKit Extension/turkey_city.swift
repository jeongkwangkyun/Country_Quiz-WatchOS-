...
class turkey_city: WKInterfaceController {
    var tmp=0
    
    @IBOutlet weak var total_8: WKInterfaceLabel!
    override func awake(withContext context: Any?) {
        super.awake(withContext: context)
        if let count = context as? Int{
            total_8.setText("Score : \(count)")
            tmp=count
        }
    ...
    @IBAction func turkey_correct() {
        tmp=tmp+1
        pushController(withName: "final", context: tmp)
    }
    
    @IBAction func turkey_wrong() {
        pushController(withName: "final", context: tmp)
    }
}
